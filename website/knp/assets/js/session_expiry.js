    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.expired) {
                ...
                $("#countdown").html("Phew!");
            } else if (response.seconds_remaining < 1200 && response.seconds_remaining > 900) { // 20 minutes left, start showing timer.
                $("#countdown").removeClass("orange");
                $("#countdown").removeClass("red");
                $("#countdown").removeClass("color-flashing");
                $("#countdown").html(response.time_remaining + "   <a href='#' class='refresh-button' onclick='refreshSession()'><i class='fas fa-sync-alt'></i></a>");
            } else {
                if (response.seconds_remaining < 300) { // Fewer than 5 minutes remaining. Really get gnarly.
                    $("#countdown").html("Your session will expire in: " + response.time_remaining + "   <a href='#' class='refresh-button' onclick='refreshSession()'><i class='fas fa-sync-alt'></i></a>");
                    $("#countdown").removeClass("orange");
                    $("#countdown").addClass("red");
                    $("#countdown").addClass("color-flashing");
                } else if (response.seconds_remaining < 900) { // Fewer than 15 minutes remaining. Start being more serious.
                    $("#countdown").html("Your session will expire in: " + response.time_remaining + "   <a href='#' class='refresh-button' onclick='refreshSession()'><i class='fas fa-sync-alt'></i></a>");
