...
def count_recurring(course, student_path, teacher_path, messages_path, verbose=False, overwrite=False):
    all_messages = read_json_messages(
        course, student_path, teacher_path, messages_path, overwrite=overwrite)
        ...
        for message in messages:
            ...
            try:
                ...
                if language == "dutch":
                    ...
                    ) and word.lower() not in filter_current and len(word) > 3]
                    ...
                    tokens = [nltk.word_tokenize(
                        sentence, language=language) for sentence in sentences]

                    filtered_tokens = []

                    for token_list in tokens:
                        filtered_tokens.extend(
                            [word for word in token_list if word.lower() not in filter_current])

                    pos_tags = nltk.pos_tag(filtered_tokens, lang="eng")

                    words = [word for word, pos in pos_tags if pos in [
                        'NN', 'NNS', 'NNP', 'NNPS'] and len(word) > 3]
    ...
    for language in counters.keys():
        for message_type in counters[language].keys():
            if message_type != "combined":
                counters[language]["combined"].update(
                    counters[language][message_type])
...
def get_counter(language, message_type, counters, how_many=None, verbose=False):
    if language in counters and message_type in counters[language]:
        ...
        if verbose:
            print(
                f"Invalid language or message type. Language ok?: {lang_ok}, Type ok?: {type_ok}")
