...
def run_application(app_type, comments, none_indices, num_pos, num_neg, device="cuda"):
    ...
    for app in apps:
        ...
        accuracy = {"pos": pos_accuracy,
                    "neg": neg_accuracy, "add": add_accuracy}
...
def run_sentiment_analysis(course, read_paths, store_paths, ai=False, verbose=False, overwrite=False, device="cuda"):
    ...
    messages_read = read_json_messages(
        course, student_path, teacher_path, message_path, overwrite, return_entries=False)
    student_entry_sentiment_numerical = messages_read['sentiment']

    # Convert numerical values to string values.
    student_entry_sentiment = sentiment_list_convert(
        student_entry_sentiment_numerical)
    # Count the occurence of each value for comparison to AI, or return after.
    student_entry_sentiment_counter = sentiment_counter(
        student_entry_sentiment)
    ...
    if ai:
        ...
        none_indices = [index for index,
                        none in enumerate(add) if none == "_none_"]
        ...
        averaged_result = [sentiment_average(
            scores) for scores in zip(*results)]
        ...
        average_positive_comment_accuracy = comment_accuracy(
            'positive', pos_results)
        average_negative_comment_accuracy = comment_accuracy(
            'negative', neg_results)
        average_additional_comment_accuracy = comment_accuracy(
            'additional', add_results)
        ...
        ai_entry_sentiment = [sentiment_average(scores) for scores in zip(
        ...
        if verbose:
            ...
            print(
                f"NEG: {neg_results}   | Weighted accuracy: {average_negative_comment_accuracy}")
            print(
                f"ADD: {add_results}   | Weighted accuracy: {average_additional_comment_accuracy}")
        ...
        entry_accuracies = entry_accuracy(
            student_entry_sentiment, ai_entry_sentiment)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run sentiment analysis script")

    parser.add_argument("--course", type=str,
                        default="SSVT", help="Course name")
    parser.add_argument("--ai", action="store_true",
                        default=True, help="Enable AI")
